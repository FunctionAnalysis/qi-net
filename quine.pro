...
parse:connected(comp2,comp,computer):-parse:connected(computer2,computer,comp),input(computer2,computer,comp).
parse:device(computer,comp2,comp).
parse:device(defines,classification,port).
parse:(output(classification(comp,computer2,comp2))):-input(unknown(computer,comp2,computer2)).
prolog:error_message(dde_error(Op,Msg)) -->
	[ 'DDE: ~w failed: ~w'-[Op,Msg] ].
unknown(output):-unknown(input).
classification(X):-(input(comp|[X])).
classification(unknown):-input(unknown).
classification(comp,comp2,computer):-unknown(input).
input(X,Y,Z):-port(input(X,Y,Z)).
input(X,Y,Z):-input(unknown(comp2|X),(comp|Y),(computer(Z))).
input(X,Y,Z):-parse:device(X,Y,Z).
input(X,Y,Z):-parse:connected(X,Y,Z).
input(Node,X,Y):-edge(X|Y,Node).
input(port):-fact:device(port).
input(unknown(classification(Y,Z,X))):-output(unknown(comp2,[X]),([Z]),([Y])).
input(comp):-unknown(input).
input(unknown(input)).
input(unknown):-unknown(input).
input(unknown(X,Y,Z)):-input(X,Y,Z).
output(X,Y,Z):-classification(X,Y,Z).
output(X,Y,Z):-(classification(X),(Y),(Z)).
array(node(A,B,C),edge([_]),bestf([_],9999)):-array((node(A,B,C;d(_))),port(A),input(A)).
array(Line,Node,Distance):-edge(Line|Node+Distance).
array(A;Node_x;(B;Node1,(C;Node3)):-edge(A;Node1),edge(B;Node3), edge(C;Node_x)).
node(d([prime+1=prime])).
node(d([prime+2=prime])).
node(d([prime+1=prime])).
edge(X,Y):-(array(list,([]);X,Y)).
edge(X,Y):-fact:connected(X,Y).
edge([Node1,Node2];[(C;Node3)],[_]):-array(Node1;_,Node2;C,Node3).
edge([A,B];[B,C];[C,B]):-node(3),edge([A,B,C]),distance((node + edge = Distance)),array(edge,node,Distance).
edge((_;_;_):-array((edge),node(2),node(3))).
...
'$dde_disconnect'(port(Service, Topic, _Self)) :-
	dde_service(Service, Topic, _, _, _, _).
	'$dde_disconnect'(port(Service, Topic, Handle)) :-
...
'$dde_request'(port, port(Vuln), node(Vuln), comp).
...
expand(P,l(N,F/G),Bound,Tree1,Solved,Sol):-
	F=<Bound,(bagof(M/C),(s(N,M,C) ,
			      port(Member;Vuln),(not(Member;Vuln)->[M,P],Succ)),!,succlist(G,Succ,Ts),bestf(Ts,Fl),
